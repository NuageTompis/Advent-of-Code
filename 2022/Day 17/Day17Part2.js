...
const UpdateTower = (rock) => {
  ...
  // If there is a new equilibrium
  if (floorHeight === 0) {
    if (Equilibriums.length === 0) {
      let equilibrium = {
        initialRock: "-",
        endingRock: rock.name,
        initialSequenceIndex: 0,
        endingSequenceIndex: ind % input.length,
      };
      StringEquilibriums.push(JSON.stringify(equilibrium));
      Equilibriums.push({
        ...equilibrium,
        rocksPlaced: rocks,
        heightAdded: HEIGHT,
      ...
      let lastEquilibrium = Equilibriums[Equilibriums.length - 1];
      let equilibrium = {
        initialRock: Names[(Names.indexOf(lastEquilibrium.endingRock) + 1) % 5],
        endingRock: rock.name,
        initialSequenceIndex: lastEquilibrium.endingSequenceIndex,
        endingSequenceIndex: ind % input.length,
      };
      if (StringEquilibriums.includes(JSON.stringify(equilibrium))) {
        initialIndexOfAlphaEquilibrium = StringEquilibriums.indexOf(
          JSON.stringify(equilibrium)
        );
        return true;
      } else {
        StringEquilibriums.push(JSON.stringify(equilibrium));
        Equilibriums.push({
          ...equilibrium,
          rocksPlaced:
            rocks - Equilibriums.reduce((ac, cur) => ac + cur.rocksPlaced, 0),
          heightAdded:
            HEIGHT - Equilibriums.reduce((ac, cur) => ac + cur.heightAdded, 0),
        });
      }
    }
  }
...
while (!alphaEquilibriumFound) {
  for (let j = 0; j < 5; j++) {
    ...
    if (UpdateTower(rock)) alphaEquilibriumFound = true;
  }
}

// The alpha equilibrium has now been found ! We just need to compute the overall height
let rocksPlacedInAE = 0;
let heightAddedInAE = 0;
for (let i = initialIndexOfAlphaEquilibrium; i < Equilibriums.length; i++) {
  rocksPlacedInAE += Equilibriums[i].rocksPlaced;
  heightAddedInAE += Equilibriums[i].heightAdded;
}
let rocksPlacedBeforeAE = 0;
for (let i = 0; i < initialIndexOfAlphaEquilibrium; i++) {
  rocksPlacedBeforeAE += Equilibriums[i].rocksPlaced;
}

const r = (total - rocksPlacedBeforeAE) % rocksPlacedInAE;
const n = (total - rocksPlacedBeforeAE - r) / rocksPlacedInAE;
